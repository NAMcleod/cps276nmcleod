...
class Validation{
	...
	public function checkFormat($value, $regex)
	{
		switch($regex){
			...
			//add regex's for admin info - copy/edit addContact.php/deleteContact.php to process tableADMIN info
	...
	private function email($value){
		$match = preg_match('/[-0-9a-zA-Z.+_]+@[-0-9a-zA-Z.+_]+\.[a-zA-Z]{2,4}/', $value);  
		return $this->setError($match);
	}

	private function address($value){
		$match = preg_match('/\d+\s+\w+\s+\w+/i', $value);  
		return $this->setError($match);
	}

	private function city($value){
		$match = preg_match('/^[a-zA-Z\',.\s-]{1,25}$/i', $value);  //remove (',.)? !*!
		return $this->setError($match);
	}
	
	private function dob($value){
		$match = preg_match('/^(0[1-9]|1[0-2])\/(0[1-9]|[12][0-9]|3[01])\/(19|20)\d\d$/', $value);
		return $this->setError($match);
	}
	...
	private function setError($match){
		if(!$match){
			$this->error = true;
			//echo "!*! 1 setError:" . $this->error ;
			return "error";
		}
		else {
			//echo "!*! 2 setError:" . $this->error;
			return "";
		}
	}
