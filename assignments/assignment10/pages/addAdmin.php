...
[
  ...
  "email"=>[
	"errorMessage"=>"<span style='color: red; margin-left: 15px;'>Email cannot be blank and must be a valid Email Address, not in use</span>",
    "errorOutput"=>"",
    "type"=>"text",
  ...
  "status"=>[
    "type"=>"select",
    "options"=>["admin"=>"Administrator","staff"=>"Staff"],
	"selected"=>"staff",
	"regex"=>"name"
   ]
];

function checkEmailClone($post)
{
...
function addData($post){
  ...
  /* IF EVERYTHING WORKS ADD THE DATA HERE TO THE DATABASE HERE USING THE $_POST SUPER GLOBAL ARRAY */
      ...
      if (checkEmailClone($post))
      {
          return getForm("<p>Email is already in use</p>", $elementsArr);
      }
      
      $pdo = new PdoMethods();

      $sql = "INSERT INTO tableADMIN (name, email, password, status) VALUES (:name, :email, :password, :status)";
