...
[
  ...
  "address"=>[
		"errorMessage"=>"<span style='color: red; margin-left: 15px;'>Address not Accepted</span>",
    "errorOutput"=>"",
    "type"=>"text",
  ...
  "city"=>[
		"errorMessage"=>"<span style='color: red; margin-left: 15px;'>City not Accepted</span>",
    "errorOutput"=>"",
    "type"=>"text",
		"value"=>"Ypsilanti",
		"regex"=>"city"
  ...
  "dob"=>[
		"errorMessage"=>"<span style='color: red; margin-left: 15px;'>Date not Accepted</span>",
    "errorOutput"=>"",
    "type"=>"text",
		"value"=>"06/13/1994",
		"regex"=>"dob"
...
function addData($post){
  ...
  /* IF EVERYTHING WORKS ADD THE DATA HERE TO THE DATABASE HERE USING THE $_POST SUPER GLOBAL ARRAY */
      ...
      $sql = "INSERT INTO tableSTAFF (name, phone, email, address, city, state, dob, contactUserBy, ageRange) VALUES (:name, :phone, :email, :address, :city, :state, :dob, :contactUserBy, :ageRange)";

      /* THIS TAKE THE ARRAY OF CHECK BOXES AND PUT THE VALUES INTO A STRING SEPERATED BY COMMAS  */
      if(isset($_POST['contactUserBy'])){
        $contact = "";
        foreach($post['contactUserBy'] as $v){
          $contact .= $v.",";
        }
...
$form = <<<HTML
    ...
    <!-- CONTACT OPTIONS -->
    <p>Please check all contact methods you would like (optional):{$elementsArr['contactUserBy[]']['errorOutput']}</p>
    <div class="form-check form-check-inline">
      <input class="form-check-input" type="checkbox" name="contactUserBy[]" id="contactUserBy1" value="Newsletter" {$elementsArr['contactUserBy[]']['status']['Newsletter']}>
      <label class="form-check-label" for="contactUserBy1">Newsletter</label>
    </div>

    <div class="form-check form-check-inline">
      <input class="form-check-input" type="checkbox" name="contactUserBy[]" id="contactUserBy2" value="Text" {$elementsArr['contactUserBy[]']['status']['Text']}>
      <label class="form-check-label" for="contactUserBy2">Text</label>
    </div>

    <div class="form-check form-check-inline">
      <input class="form-check-input" type="checkbox" name="contactUserBy[]" id=contactUserBy3" value="Email" {$elementsArr['contactUserBy[]']['status']['Email']}>
      <label class="form-check-label" for="contactUserBy3">Email</label>
    </div>

    <!-- AGE RANGE -->
    <p>Please select an Age Range (required):{$elementsArr['ageRange']['errorOutput']}</p>
    <div class="form-check form-check-inline">
      <input class="form-check-input" type="radio" name="ageRange" id="ageRange1" value="10-18"  {$elementsArr['eyeColor']['status']['value']['10-18']}>
      <label class="form-check-label" for="ageRange1">10-18</label>
    </div>

    <div class="form-check form-check-inline">
      <input class="form-check-input" type="radio" name="ageRange" id="ageRange2" value="19-30"  {$elementsArr['eyeColor']['status']['value']['19-30']}>
      <label class="form-check-label" for="ageRange2">19-30</label>
    </div>

    <div class="form-check form-check-inline">
      <input class="form-check-input" type="radio" name="ageRange" id="ageRange3" value="31-50"  {$elementsArr['eyeColor']['status']['value']['31-50']}>
      <label class="form-check-label" for="ageRange3">31-50</label>
    </div>

    <div class="form-check form-check-inline">
      <input class="form-check-input" type="radio" name="ageRange" id="ageRange4" value="51+"  {$elementsArr['eyeColor']['status']['value']['51+']}>
    ...
    <!-- check buttons 
    COMMENTING OUT FINANCIAL AND EYECOLOR OPTIONS
    --*!*>
    ...
    END FINANCIAL/ EYE COLOR CHECKS/RADIOS-->
    <!-- submit button -->
    <div>
